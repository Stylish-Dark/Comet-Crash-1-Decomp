...
## D014 — Generated PPU no-op/unsupported output is a hard lift gate

After applying the two known Comet VMX compatibility rewrites (`vsrab`, `vsrb`), a fresh PPU lift must contain no remaining generated `/* TODO: ... */;` instruction holes and no explicit `unsupported SPR -- no-op` fallback. Missing PPU chunks or any residual unsupported/no-op instruction stops the pipeline before build/boot.
...
## D016 — The toolchain pin is an execution gate, not documentation
