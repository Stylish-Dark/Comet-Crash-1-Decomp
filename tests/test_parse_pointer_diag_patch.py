...
class ParsePointerDiagPatchTests(unittest.TestCase):
    ...
    def test_instruments_register_sp_and_slot_lifetime(self):
        ...
        self.assertEqual(out.count('[COMET-PARSE-REG-CLOBBER]'),2+len(p.PARSE_R23_SITES))
        self.assertEqual(out.count('[COMET-PARSE-SP-CHANGE]'),1+len(p.CALLER_SITES))
        self.assertEqual(out.count('[COMET-PARSE-SLOT-CHANGE]'),1+len(p.CALLER_SITES))
