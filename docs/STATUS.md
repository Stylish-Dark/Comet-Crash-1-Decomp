...
- The exact pinned PPU lifter still leaves the two known Comet VMX holes (`vsrab`, `vsrb`); `tools/patch_ppu_lift.py` patches them alias-safely and the lift command applies that automatically. A post-patch PPU completeness audit then fails the lift if any generated `/* TODO: ... */;` instruction or explicit `unsupported SPR -- no-op` fallback remains.
...
- CI now has a Windows scaffold compile/link job against the exact locked ps3recomp commit. It applies every Comet runtime patch, runs the real pinned PPU lifter on a non-proprietary one-instruction PPC fixture, applies the same compatibility patch/completeness audit as the Comet pipeline, and then compiles/links that generated output. The SPU side remains synthetic for now.
- First-boot diagnostics are now durable: the run command mirrors combined native stdout/stderr into `logs/boot-YYYYMMDD-HHMMSS.txt`, records ELF/runtime-path metadata, and the runner emits `[boot-stage]` checkpoints through PPU entry plus the first presented guest frame. Unhandled native exceptions also record code/address, image RVA, thread and AV target into that same log.

GitHub PR run `35680385634` passed the stronger exact-pin gate: 74/74 tests passed, the real pinned PPU lifter generated code successfully on Windows, the Comet compatibility patch and PPU completeness audit accepted that generated output, and clang-cl linked `CometCrashPC.exe` successfully. Generated-source handling is byte-safe across Windows default encodings.
