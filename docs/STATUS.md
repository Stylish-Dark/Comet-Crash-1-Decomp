...
- First-boot diagnostics are now durable: the run command mirrors combined native stdout/stderr into `logs/boot-YYYYMMDD-HHMMSS.txt`, records ELF/runtime-path metadata, and the runner emits `[boot-stage]` checkpoints through PPU entry plus the first presented guest frame. `tools/boot_triage.py` classifies a saved log into the project’s runtime blocker categories while deliberately returning `unknown` for generic crash/watchdog symptoms that do not identify a subsystem. The normal `comet_port.py run` path now invokes the same classifier automatically and writes `suspected_subsystem` plus `triage_rationale` into every completed boot log.
