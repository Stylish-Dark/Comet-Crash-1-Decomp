...
- First-boot diagnostics are now durable: the run command mirrors combined native stdout/stderr into `logs/boot-YYYYMMDD-HHMMSS.txt` and emits an adjacent `.summary.json`. Triage preserves the first chronological symptom but prefers a later subsystem-specific signal when available; generic crash/watchdog symptoms remain `unknown`. Evidence includes first-frame state, outcome, timeout/interruption status, subsystem/rationale and exact build provenance. Provenance schema v2 cryptographically binds the actual native EXE to the tracked source snapshot, exact toolchain pin and complete HLE baseline; normal runs reject mismatches. The summary also binds to the finalized text log by SHA-256, and `tools/verify_boot_bundle.py` independently re-derives its runtime fields. Native crashes additionally record boot stage, frame count, last HLE breadcrumb and whether an access-violation target lies inside guest VM memory. Ctrl+C and optional run timeouts preserve a final diagnostic footer; timeouts are strictly bounded by terminate/kill escalation.

GitHub Actions run `35969371258` is the current authoritative pre-boot gate: **131/131 tests passed**, `compileall` and repository safety passed on Linux and Windows, the real pinned PPU lifter generated its fixture successfully, every deterministic Comet runtime patch applied, the byte-safe PPU audit accepted the lift, clang-cl/Ninja linked `CometCrashPC.exe`, and the actual linked Windows EXE successfully completed the provenance round-trip.

The pinned D3D12 patch now also adds exact failure diagnostics to nine previously generic/silent initialization exits (queue, swap-chain interface/backbuffers, descriptor heaps, command allocators/list, fence/event and root-signature creation), so the first real graphics failure should retain its API/HRESULT detail.
