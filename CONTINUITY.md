...
Do not silently move this pin. Runtime source patchers deliberately fail on upstream drift. Analysis, lift and build also verify that the local ps3recomp checkout is a Git checkout at exactly this locked commit.
...
- `tools/audit_ppu_lift.py` — post-compatibility PPU completeness gate; a fresh lift fails if expected PPU chunks are missing, if any generated `/* TODO: ... */;` instruction remains, or if ps3recomp emits an explicit `unsupported SPR -- no-op` fallback.
...
- Boot diagnostics now write `logs/boot-YYYYMMDD-HHMMSS.txt` by default and include stage markers through PPU entry plus the first presented guest frame. The Windows runner also records unhandled native exception code/address, image RVA, thread and AV target into the same durable log.
